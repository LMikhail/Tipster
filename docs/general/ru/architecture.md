...
graph TD;
    ...
    subgraph "Runtime (Этап исполнения)"
        ...
        D["<b>Движок выполнения</b><br/>Унификация и управление"];
        CM["<b>Комбинаторная машина</b><br/>Поиск с возвратами<br/>Backtracking"];
    ...
    D -- "(4) Делегирует поиск" --> CM;
    CM -- "(5) Обращается за правилами" --> C;
    C -- "(6) Возвращает правила" --> CM;
    CM -- "(7) Возвращает решения" --> D;
    D -- "(8) Возвращает результат" --> E;
    E -- "Результаты" --> F;
...
*   **Декомпозиция термов:** Преобразует сложные Clojure-структуры в оптимизированную форму для эффективной унификации. Ключевые этапы:
    *   **Структурный анализ:** Разложение составных термов на функтор и аргументы
    *   **Рекурсивная обработка:** Декомпозиция вложенных коллекций (векторы, мапы, сеты)
    *   **Метаданные унификации:** Создание индексов и сигнатур для быстрого сопоставления
    *   **PKVT-интеграция:** Преобразование в атомарные записи для хранения
*   **Взаимодействие:** Активно обращается к [Базе Знаний (PKVT)](#база-знаний-pkvt) для получения фактов и правил, а также делегирует поиск решений [Комбинаторной машине](#комбинаторная-машина).
*   **Выход:** Ленивая последовательность решений, где каждое решение — это набор связанных переменных.

**Архитектурное решение ("Почему"):**
1.  **Проверенное ядро:** В основе движка лежат десятилетиями проверенные алгоритмы: унификация Робинсона для сопоставления термов. Это обеспечивает корректность поиска.
...
3.  **Отделение "унификации" от "поиска":** Движок отвечает за *унификацию* термов и управление состоянием. Он делегирует *поиск с возвратами* [Комбинаторной машине](#комбинаторная-машина), что позволяет каждому компоненту быть специализированным и эффективным.

### Комбинаторная машина

**Отвечает за:** Реализацию алгоритмов поиска решений в пространстве логических возможностей. Это независимый архитектурный блок, который инкапсулирует различные стратегии обхода дерева поиска.

*   **Вход:** Логическая цель и набор доступных правил из [Базы Знаний](#база-знаний-pkvt).
*   **Алгоритмы:** Поиск с возвратами (backtracking), поиск в глубину, поиск в ширину, итеративное углубление.
*   **Выход:** Последовательность потенциальных решений для [Движка выполнения](#движок-выполнения).

**Архитектурное решение ("Почему"):**
1.  **Модульность поиска:** Выделение поиска в отдельный компонент позволяет экспериментировать с различными стратегиями (DFS, BFS, A*) без изменения логики унификации.
2.  **Масштабируемость:** Комбинаторная машина может быть оптимизирована для конкретных типов задач или даже распределена на несколько узлов.
3.  **Переиспользование:** Один и тот же алгоритм поиска может использоваться для разных типов логических задач, от простых запросов до сложных планирования и оптимизации.
